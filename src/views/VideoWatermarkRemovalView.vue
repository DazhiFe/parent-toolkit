        ...
        <div class="space-y-4">
          <div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
              <label class="flex items-center p-4 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-primary-500 transition-colors" :class="{ 'border-primary-500 bg-primary-50 dark:bg-primary-900/20': watermarkMode === 'static' }">
                <input type="radio" v-model="watermarkMode" value="static" class="mr-3">
                <div>
                  <p class="font-medium text-gray-800 dark:text-white text-sm">固定水印</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">位置固定不变</p>
                </div>
              </label>
              <label class="flex items-center p-4 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-primary-500 transition-colors" :class="{ 'border-primary-500 bg-primary-50 dark:bg-primary-900/20': watermarkMode === 'floating' }">
                <input type="radio" v-model="watermarkMode" value="floating" class="mr-3">
                <div>
                  <p class="font-medium text-gray-800 dark:text-white text-sm">飘动水印</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">位置会移动</p>
                </div>
              </label>
              <label class="flex items-center p-4 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-primary-500 transition-colors" :class="{ 'border-primary-500 bg-primary-50 dark:bg-primary-900/20': watermarkMode === 'crop' }">
                ...
                <div>
                  <p class="font-medium text-gray-800 dark:text-white text-sm">裁剪模式</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">裁剪边缘区域</p>
                </div>
              </label>
            </div>
          </div>

          <div v-if="watermarkMode === 'floating'" class="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
            <div class="flex items-start gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-orange-600 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <div>
                <p class="text-sm font-medium text-orange-800 dark:text-orange-300">飘动水印处理说明</p>
                <p class="text-sm text-orange-700 dark:text-orange-400 mt-1">
                  飘动水印会在视频中移动位置，需要逐帧跟踪和处理。当前在线工具无法完美处理此类水印，建议使用以下专业方案：
                </p>
                <ul class="text-sm text-orange-700 dark:text-orange-400 mt-2 space-y-1">
                  <li>• 使用 <strong>HitPaw Watermark Remover</strong> 等专业软件</li>
                  <li>• 使用 <strong>Adobe Premiere Pro</strong> 配合遮罩跟踪</li>
                  <li>• 使用 <strong>达芬奇 Resolve</strong> 的智能遮罩功能</li>
                  <li>• 使用 <strong>在线AI工具</strong> 如 watermark-remover.io</li>
                </ul>
              </div>
            </div>
          </div>

          <div v-if="watermarkMode === 'static'" class="grid grid-cols-2 gap-4">
            <div>
